		...
		<div class="hero-unit span13"
			...
			<div>
				...
				<p
					...
					<td><a class="btn btn-info btn-small"
						href="issueHistoryListPageInEdit.html?id=${itwTaskBug.id}&&projectId=${projectId}" style="float:right; margin-right:5px">History</a></td>
				</p>
			...
			<div id="tabs">
				...
				<div id="Issue-Details">
					<p>
                        <div class="parkunpark" style="position:absolute; margin-left: 91px; margin-top: -6px;">
						<c:if
							test="${itwIssueParkBean.status == 0 || itwIssueParkBean.status==null}">

							<form:form method="POST"
								action="unPark.html?projectid=${itwTaskBug.projectid}">

								<input type="submit" class="btn btn-success btn-small"
									value="UnPark" align="right" />
						...
						<c:if test="${itwIssueParkBean.status == 1}">
							<form:form method="POST"
								action="park.html?issueId=${itwIssueParkBean.id}&&createdDate=${itwIssueParkBean.createdTime}&&projectid=${itwTaskBug.projectid}&&reason=${reason}">

								<a href="#pop1"><input type="button"
									class="btn btn-danger btn-small" value="Park" align="right" style="padding: 3.5px 10px;" /></a>

								<td>Your Time Starts Now</td>
								<table>
								...
								<div id="pop1" class="pop-up">
									<div class="popBox">
										<div class="popScroll">
											<p>
												<form:input path="parkreason" type="text"
													placeholder="Enter reason for park..."
													style="padding: 5px; width: 314px; margin-top: 10px;" />
												<input type="submit" class="btn btn-danger btn-small"
													value="Sumbit" align="right" />
											</p>
										</div>
						...
						<form:form method="POST" action="updateItwTaskBug1.html?projectId=${projectId}">
							<div style="position: absolute; top: 49px;">
								<input type="submit" class="btn btn-success btn-small"
									value="Update Issue" />
							</div>
<br>
							<table
								...
								<tr>
									...
									<td><form:select path="statusid">
											<c:set var="statusiddisplaytemp">${itwTaskBug.statusid}</c:set>
								...
								<tr>
									...
									<form:option value=" " label="Select Depend" />
                        ...
                        <tr>
                         <td>Approved&nbsp;By</td>
					      <td><form:select path="approvedBy"><form:option value=" " label="Select Userame" />
								...
								<c:forEach items="${ItwUserBean}"
												var="itwUserBean"  >
								             	...
								             	<c:choose>
													...
													<c:otherwise>
														<option value="${itwUserBean.id}">${itwUserBean.userid}</option>
													</c:otherwise>
												</c:choose>
				...
				<div id="Attach-Documents">
					...
					<form:form method="post" action="savedoc.html?projectId=${projectId}"
							...
							<c:forEach items="${documentList}" var="document">
								<tr>
									...
									<td><a class="btn btn-primary btn-small"
										...
										href="deleteItwDocument.html?id=${document.id}&&taskId=${document.taskId}&&projectId=${projectId}">Delete</a></td>
								</tr>
							</c:forEach>
						</table>
					</c:if>
				...
				<div id="Capture-Image">
					<form:form id="imageSubmitForm" method="POST"
						action="captureImage.html?id=${itwTaskBug.id}&&projectId=${projectId}">
							...
							<c:forEach items="${images}" var="image">
								<tr>
									...
									<td><a class="btn btn-primary btn-small"
										...
										<a class="btn btn-danger btn-small"
										href="deleteItwImage.html?id=${image.id}&&taskId=${image.taskId}&&projectId=${projectId}">Delete</a></td>
								</tr>
							</c:forEach>
						</table>
					</c:if>
